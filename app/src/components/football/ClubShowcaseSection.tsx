...
export function ClubShowcaseSection() {
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-white py-12 sm:py-16">
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8">
        ...
        >
          <p className="text-xs font-semibold uppercase tracking-[0.45em] text-black/45">
            Football
          </p>
          <h2 className="mt-3 text-3xl font-black tracking-[0.12em] text-black sm:text-5xl">
            SHOP BY CLUB
          </h2>
          <p className="mt-3 text-sm text-black/65 sm:text-lg">Scroll Right To Explore</p>
        </motion.div>
        ...
        <div className="relative mt-10">
          ...
          <div
            ref={scrollRef}
            className="scrollbar-hide flex gap-5 overflow-x-auto pb-3 snap-x snap-mandatory"
          >
            {clubBadges.map((club, index) => (
              <motion.div
                ...
                className="group flex w-[72px] shrink-0 flex-col items-center snap-start sm:w-[86px]"
              >
                <Link
                  to={`/search?q=${encodeURIComponent(club.name)}`}
                  className="flex flex-col items-center"
                >
                  <div className={`flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br ${club.colors} p-[2px] shadow-[0_6px_20px_rgba(0,0,0,0.08)] transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_10px_28px_rgba(0,0,0,0.16)] sm:h-16 sm:w-16`}>
                    <div className="flex h-full w-full items-center justify-center rounded-full bg-white p-2.5 transition-transform duration-300 group-hover:scale-105">
                      <img
                        src={club.logo}
                        alt={club.name}
                  ...
                  <p className="mt-2 text-center text-[10px] font-semibold leading-tight text-black/70 transition-colors group-hover:text-black sm:text-xs">
                    {club.name}
                  </p>
                </Link>
              </motion.div>
          ...
          <div className="mt-4 flex justify-end gap-2">
            <button
              type="button"
              onClick={() => scroll('left')}
              className="rounded-full border border-black/10 bg-white p-2.5 text-black shadow-sm transition hover:-translate-y-0.5 hover:bg-black hover:text-white"
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-2 lg:gap-6">
          {clubFeatures.map((club, index) => (
            <motion.article
              ...
              className="group relative h-[270px] overflow-hidden rounded-[28px] bg-[#111827] shadow-[0_20px_60px_rgba(0,0,0,0.12)] sm:h-[330px]"
            >
              <img
                src={club.image}
                alt={club.name}
              ...
              <div className="relative flex h-full flex-col justify-end p-6 sm:p-8">
                ...
                <h3 className="mt-2 text-3xl font-black tracking-wide text-white sm:text-4xl">
                  {club.name}
                </h3>
                ...
                <Link
                  to={club.link}
                  className="mt-5 inline-flex w-fit items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-black transition hover:gap-3"
                >
                  Explore Club
                  <ArrowRight size={16} />
                </Link>
